# Hey, I'm Victor Buarque 👋 🇧🇷

Full Stack Engineer specialized in front-end, focused on building scalable, high-performance and user-centered digital products.

## 🚀 About Me

- 👨‍💻 Full Stack Engineer at **Kreatives.io**
- ⚛️ Specialized in **React, Next.js and modern front-end architecture**
- 🧩 Experience across the full stack, from UI to backend integrations
- 📱 Developing cross-platform mobile apps using **React Native**
- 🌐 Improving my English for global opportunities
- 🤝 Open to collaborate on innovative and impactful projects

---

## 💼 What I Do

- Build scalable and maintainable front-end architectures  
- Create high-performance and responsive interfaces  
- Develop mobile-first applications  
- Integrate complex APIs and business rules  
- Work across the stack when needed (frontend + backend)  
- Improve UX/UI with clean and efficient code  

---

## 🧠 Tech Stack

<div align="center">

<img alt="JavaScript" height="30" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-plain.svg">
<img alt="TypeScript" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" />
<img alt="React" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" />
<img alt="Next.js" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" />
<img alt="React Native" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" />
<img alt="TailwindCSS" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" />
<img alt="Sass" height="30" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sass/sass-original.svg" />
<img alt="Bootstrap" height="30" src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bootstrap/bootstrap-original.svg">

</div>

---

## ⭐ Highlight Projects

### 🔥 Chaintix Platform  
🔗 https://www.chaintix.co/  
🔗 https://github.com/kreatives-io/chaintix.co  

**Overview**  
AI-powered ticketing platform with blockchain-based verification and a mobile-first experience.

**My Contributions**  
- Developed scalable front-end using React and Next.js  
- Implemented mobile-first UI for cross-device consistency  
- Integrated APIs for ticket validation and user flows  
- Focused on performance optimization and UI scalability  

**Tech Stack**  
React, React Native, Next.js, TypeScript, TailwindCSS, Clerk, Sanity, ThirdWeb 

**Impact**  
- Improved usability across mobile devices  
- Enhanced performance and loading times  
- Contributed to a modern ticketing solution with scalable architecture  

---

### 🔥 Amanda Cretella Platform  
🔗 https://amandacretella.com/  
🔗 https://github.com/kreatives-io/amandacretella.com  

**Overview**  
Membership-based platform where users access exclusive content through paid subscriptions.

**My Contributions**  
- Developed front-end using React and Next.js  
- Built authenticated flows and protected routes  
- Implemented exclusive client area with gated content  
- Focused on onboarding experience and engagement  

**Tech Stack**  
Next.js, React, Clerk, TailwindCSS  

**Impact**  
- Enabled subscription-based business model  
- Improved user retention through exclusive content  
- Delivered secure and seamless user experience  

---

### 🔥 Edvaldo Raw Portfolio  
🔗 https://www.edvaldoraw.com/  
🔗 https://github.com/kreatives-io/edvaldoraw.com  

**Overview**  
High-performance portfolio for an award-winning cinematographer focused on visual storytelling.

**My Contributions**  
- Developed front-end using Next.js with performance focus  
- Integrated Sanity CMS for dynamic content management  
- Optimized media loading and rendering  

**Tech Stack**  
Next.js, TypeScript, Sanity CMS  

**Impact**  
- Delivered fast-loading and visually rich experience  
- Enabled easy content management via CMS  

---

## 📊 Dev Card

<div align="center">
  <a href="https://app.daily.dev/victorgabriel">
    <img src="https://api.daily.dev/devcards/v2/am1o54JGPBjxtqvbHF7fT.png?type=wide&r=xee" width="600" alt="Victor Gabriel Dev Card"/>
  </a>
</div>

---

## 🌎 Let's Connect

<div align="center">

<a href="mailto:vcgabriel38@gmail.com">
  <img src="https://img.shields.io/badge/Gmail-333?style=for-the-badge&logo=gmail&logoColor=white">
</a>
<a href="https://www.linkedin.com/in/victorbuarque">
  <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white">
</a>
<a href="https://instagram.com/victbuarque">
  <img src="https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white">
</a>

</div>

---

## ⚡ Fun Fact

I enjoy turning complex problems into simple, intuitive and high-performance interfaces.

---

⭐ From [VictorBuarque](https://github.com/victorbuarque)
